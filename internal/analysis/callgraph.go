...
func GenerateCallGraph(root string) (*CallGraph, error) {
	...
	// Optimization maps
	packageMap := make(map[string][]*CallGraphNode)
	methodMap := make(map[string][]*CallGraphNode)
		...
		for _, decl := range f.Decls {
			if fn, ok := decl.(*ast.FuncDecl); ok {
				...
				if fn.Recv != nil {
					...
					methodMap[fn.Name.Name] = append(methodMap[fn.Name.Name], node)
				} else {
					// Function
				...
				packageMap[fullPkg] = append(packageMap[fullPkg], node)
			}
		}
		return nil
	})
					...
					case *ast.SelectorExpr:
						...
						if xIdent, ok := fun.X.(*ast.Ident); ok {
							...
							if importPath, isImport := imports[xIdent.Name]; isImport {
								...
								calleeID = resolveExternalCall(packageMap, importPath, sel)
								if calleeID == "" {
								...
								candidates := findMethodsByName(methodMap, sel)
								if len(candidates) == 1 {
...
func resolveExternalCall(packageMap map[string][]*CallGraphNode, importPath string, funcName string) string {
	...
	// Optimize: Iterate only over packages that match the suffix of importPath
	for pkgName, nodes := range packageMap {
...
func findMethodsByName(methodMap map[string][]*CallGraphNode, methodName string) []*CallGraphNode {
	// O(1) lookup
