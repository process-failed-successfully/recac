...
func resolveExternalCall(cg *CallGraph, importPath string, funcName string) string {
	...
	// Rule: Longest package path match first.
	// Tie-breaker: Lexicographical order of ID.
	sort.Slice(candidates, func(i, j int) bool {
		nodeI := cg.Nodes[candidates[i]]
		nodeJ := cg.Nodes[candidates[j]]
		if len(nodeI.Package) != len(nodeJ.Package) {
			return len(nodeI.Package) > len(nodeJ.Package)
		}
