...
func TestProcessJiraTicket_WithRepoURL(t *testing.T) {
	...
	cfg := SessionConfig{
		...
		IsMock:        true,
		Cleanup:       false,
		MaxIterations: 2, // Set finite iterations to prevent infinite loop on failure
	}
...
// TestRunWorkflow_Normal was removed as it duplicates TestRunWorkflow_NormalMode_Mocked in workflow_run_test.go
// and was causing overlayfs errors by not using the proper mock docker client.
