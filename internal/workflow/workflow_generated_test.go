...
func (m *MockSessionManager) StartSession(name, goal string, command []string, cwd, project string) (*runner.SessionState, error) {
...
func TestRunWorkflow_Detached_Error(t *testing.T) {
	...
	t.Run("StartSession fails", func(t *testing.T) {
		mockSM.On("StartSession", "test", "", mock.Anything, mock.Anything, mock.Anything).Return(nil, errors.New("failed to start")).Once()
