...
func TestDockerSpawner_Spawn_ImageFlag(t *testing.T) {
	...
	// Use done channel and sync.Once for synchronization to ensure goroutine completes
	done := make(chan struct{})
	var once sync.Once

	// Return an error to force early exit in the goroutine, which is sufficient for this test
	...
	// Wait for Exec call (with increased timeout for CI)
	select {
	...
	// Wait for goroutine to finish (reach LoadSession)
	select {
	case <-done:
		// Success
	case <-time.After(30 * time.Second):
