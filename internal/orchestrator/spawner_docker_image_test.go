...
func TestDockerSpawner_Spawn_ImageFlag(t *testing.T) {
	...
	// Wait for goroutine to complete (LoadSession is the last step in failure path)
	select {
	case <-done:
		// Success
	case <-time.After(5 * time.Second):
		t.Log("Timed out waiting for LoadSession (goroutine cleanup), but Exec was verified")
	}
}
