...
func (s *DockerSpawner) Spawn(ctx context.Context, item WorkItem) error {
	...
	go func() {
		...
		// Use shellquote ONLY for command arguments, not environment variables
		cmdStr := shellquote.Join(agentCmd...) + " --allow-dirty"
		cmdStr += " && echo 'Recac Finished'"

		// Run via sh -c to allow chaining '&& echo ...'
		cmd := []string{"/bin/sh", "-c", cmdStr}

		s.Logger.Info("Executing agent command", "item", item.ID)
		output, execErr := s.Client.ExecWithEnv(context.Background(), containerID, cmd, env)
