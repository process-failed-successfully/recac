...
func (m *MockDockerClient) RunContainerWithLabels(ctx context.Context, image, workspace string, binds []string, labels map[string]string, env []string, user string) (string, error) {
	args := m.Called(ctx, image, workspace, binds, labels, env, user)
	return args.String(0), args.Error(1)
}
