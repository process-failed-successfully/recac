...
func (m *MockDockerClient) RunContainerWithLabels(ctx context.Context, imageRef string, workspace string, extraBinds []string, ports []string, user string, labels map[string]string) (string, error) {
	args := m.Called(ctx, imageRef, workspace, extraBinds, ports, user, labels)
	return args.String(0), args.Error(1)
}
...
func TestDockerSpawner_Spawn_Success(t *testing.T) {
	...
	// Mock expectations
	mockDocker.On("RunContainerWithLabels", ctx, "test-image", mock.AnythingOfType("string"), mock.Anything, mock.Anything, "", mock.MatchedBy(func(l map[string]string) bool {
		return l["created-by"] == "recac-orchestrator" && l["work-item"] == "TICKET-1"
