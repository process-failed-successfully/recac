...
func (m *MockDockerClient) RunContainerWithLabels(ctx context.Context, image, workspace string, binds, ports, env []string, user string, labels map[string]string) (string, error) {
	args := m.Called(ctx, image, workspace, binds, ports, env, user, labels)
	return args.String(0), args.Error(1)
}
...
func TestDockerSpawner_Spawn_Success(t *testing.T) {
	...
	// Mock expectations
	mockDocker.On("RunContainerWithLabels", ctx, "test-image", mock.AnythingOfType("string"), mock.Anything, mock.Anything, mock.Anything, "", mock.Anything).Return("container123", nil)
...
func TestDockerSpawner_ShellInjection(t *testing.T) {
	...
	client.On("RunContainerWithLabels", mock.Anything, "recac-agent:latest", mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return("container-123", nil)
...
func TestDockerSpawner_EnvPropagation(t *testing.T) {
	...
	client.On("RunContainerWithLabels", mock.Anything, "recac-agent:latest", mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return("container-env", nil)
	sm.On("SaveSession", mock.Anything).Return(nil)
	sm.On("LoadSession", mock.Anything).Return(&runner.SessionState{}, nil)
