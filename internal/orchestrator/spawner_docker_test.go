...
func (m *MockDockerClient) ExecWithEnv(ctx context.Context, containerID string, cmd []string, env []string) (string, error) {
...
func TestDockerSpawner_Spawn_Success(t *testing.T) {
	...
	// Verify ExecWithEnv includes git identity and project ID env vars in the ENV slice
	mockDocker.On("ExecWithEnv", mock.Anything, "container123", mock.Anything, mock.MatchedBy(func(env []string) bool {
		// Env vars should be passed as KEY=VALUE
		hasProjectID := false
		hasGitAuthor := false
		for _, e := range env {
			...
			if e == "GIT_AUTHOR_NAME=RECAC Agent" {
				hasGitAuthor = true
			}
		}
		return hasProjectID && hasGitAuthor
	})).Return("output", nil)
...
func TestDockerSpawner_ShellInjection(t *testing.T) {
	...
	// Capture the command passed to ExecWithEnv using a channel for synchronization
	capturedCmdChan := make(chan []string, 1)
	client.On("ExecWithEnv", mock.Anything, "container-123", mock.Anything, mock.Anything).Run(func(args mock.Arguments) {
...
func TestDockerSpawner_EnvPropagation(t *testing.T) {
	...
	// Capture the Env passed to ExecWithEnv
	capturedEnvChan := make(chan []string, 1)
	client.On("ExecWithEnv", mock.Anything, "container-env", mock.Anything, mock.Anything).Run(func(args mock.Arguments) {
	...
	assert.True(t, foundMaxIter, "Should propagate RECAC_MAX_ITERATIONS from host via Env")
	assert.True(t, foundManagerFreq, "Should propagate RECAC_MANAGER_FREQUENCY from host via Env")
}
