...
func TestDockerSpawner_EnvInjection_Vulnerability(t *testing.T) {
	...
	client.On("ExecWithEnv", mock.Anything, "container-sec", mock.Anything, mock.Anything).Run(func(args mock.Arguments) {
		capturedCmdChan <- args.Get(2).([]string)
		capturedEnvChan <- args.Get(3).([]string)
	}).Return("Success", nil)
	...
	// 1. Verify that the command string does NOT contain the malicious variable definition.
	// It should be completely moved to the Env slice.
	assert.NotContains(t, cmdStr, "MALICIOUS_VAR", "Command string should not contain env var definitions")

	// 2. Verify that the Env slice contains the malicious variable with the exact payload.
	// Since we are passing it via Env, it should be raw (not shell escaped).
	expectedEnvEntry := "MALICIOUS_VAR=" + maliciousPayload
	assert.Contains(t, capturedEnv, expectedEnvEntry, "Env slice should contain the raw malicious payload")
}
