...
func TestDockerSpawner_EnvInjection_Vulnerability(t *testing.T) {
	...
	// Verify that the payload is actually there but escaped
	// shellquote escapes ' as '\'' inside single quotes.
	// So "'; echo PWNED; '" becomes '\''; echo PWNED; '\'''
	// And the whole env export: export MALICIOUS_VAR='...'
	assert.Contains(t, cmdStr, "echo PWNED", "The payload should be present in the command string")

	// We can check for the escaped version of the quote
	// It should definitely contain '\''
	assert.Contains(t, cmdStr, "'\\''", "Command string should contain escaped quotes")
}
