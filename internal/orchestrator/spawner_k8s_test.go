...
func TestK8sSpawner_Spawn_PropagatesEnvVars(t *testing.T) {
	...
	assert.Equal(t, "/workspace", envMap["HOME"], "HOME should be /workspace")
...
func TestK8sSpawner_Spawn_Lifecycle(t *testing.T) {
	...
	t.Run("Create Success", func(t *testing.T) {
		...
		// Verify command
		assert.Contains(t, container.Args[0], "/usr/local/bin/recac-agent")
		assert.Contains(t, container.Args[0], "--verbose")
		assert.Contains(t, container.Args[0], "--image \"recac-agent:latest\"")
