...
func TestPlaybackModel_Update(t *testing.T) {
	...
	if !ok {
		t.Fatalf("Model is not PlaybackModel after resize, got %T", model)
	}
	...
	m, ok = model.(PlaybackModel)
	if !ok {
		t.Fatalf("Model is not PlaybackModel after enter, got %T", model)
	}
	if !m.viewingDetails {
	...
	m, ok = model.(PlaybackModel)
	if !ok {
		t.Fatalf("Model is not PlaybackModel after esc, got %T", model)
	}
	if m.viewingDetails {
