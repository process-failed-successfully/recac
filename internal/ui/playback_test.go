...
func TestPlaybackModel_Update(t *testing.T) {
	...
	model, _ = model.Update(tea.KeyMsg{Type: tea.KeyEnter}) // Enter details view
	m = model.(PlaybackModel)
	if !m.viewingDetails {
		t.Error("Failed to enter details view")
	}
	_, cmd = model.Update(tea.KeyMsg{Type: tea.KeyCtrlC})
	if cmd == nil {
		t.Error("Ctrl+C should return a Quit command in details view")
	}
}
