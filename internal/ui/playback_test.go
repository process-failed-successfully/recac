...
func TestPlaybackModel_Update_Details_CtrlC(t *testing.T) {
	entries := []LogEntry{
		{Time: time.Now(), Level: "INFO", Msg: "Line 1", Content: "Details 1"},
	}
	// Initial model is PlaybackModel struct
	...
	m := model.(PlaybackModel)
	if !m.viewingDetails {
		t.Fatal("Failed to enter details view")
	}
	...
	// We can't compare functions, but we can execute it and check the message
	msg := cmd()
