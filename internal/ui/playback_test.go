...
`
	...
	// 2. Parse
	entries, err := ParseLogLines([]byte(jsonl))
...
func updateModel(m tea.Model, msg tea.Msg) (PlaybackModel, tea.Cmd) {
	updated, cmd := m.Update(msg)
	return updated.(PlaybackModel), cmd
}

func TestPlaybackModel_Update(t *testing.T) {
	entries := []LogEntry{
		{Level: "INFO", Msg: "Item 1", Content: "Details 1"},
		{Level: "WARN", Msg: "Item 2", Content: "Details 2"},
	}
	...
	assert.Contains(t, m.View(), "Item 1")
	assert.NotContains(t, m.View(), "Details 1")

	// 2. Select Item (Enter) -> Details View
	m, _ = updateModel(m, tea.KeyMsg{Type: tea.KeyEnter})
	assert.Contains(t, m.View(), "Details 1")
	assert.Contains(t, m.View(), "Entry Details")

	// 3. Scroll in Details View (just ensuring no panic)
	m, _ = updateModel(m, tea.KeyMsg{Type: tea.KeyDown})

	// 4. Back to List View (Esc)
	m, _ = updateModel(m, tea.KeyMsg{Type: tea.KeyEsc})
	assert.Contains(t, m.View(), "Item 1")
	assert.NotContains(t, m.View(), "Details 1") // Should be back to list
