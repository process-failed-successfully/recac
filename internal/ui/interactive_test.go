...
func TestInteractiveModel_ClearHistory(t *testing.T) {
	m := NewInteractiveModel(nil, "", "")
	m.conversation("msg1", false)
	m.conversation("msg2", true)
	...
	// It re-adds a cleared msg
	if len(m.messages) != 1 {
		t.Errorf("Expected 1 message (cleared status), got %d", len(m.messages))
	}
...
func TestInteractiveModel_Update_ModeCmd_Filtering(t *testing.T) {
	cmds := []SlashCommand{
		{Name: "/foo", Description: "Foo", Action: nil},
		{Name: "/bar", Description: "Bar", Action: nil},
	}
	m := NewInteractiveModel(cmds, "", "")
	m.setMode(ModeCmd)
	m.showList = true

	// Filter "/fo" (must start with / to trigger filter logic in Update)
	m.textarea.SetValue("/fo")
	// Trigger update to refresh list
	msg := tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune{'o'}}
	newM, _ := m.Update(msg)
	m = newM.(InteractiveModel)
	...
	if !foundFoo {
		t.Error("List should contain /foo")
	}
	if foundBar {
		t.Error("List should NOT contain /bar")
	}
}

func TestInteractiveModel_Update_ShellMode(t *testing.T) {
	m := NewInteractiveModel(nil, "", "")
	m.setMode(ModeShell)
	m.textarea.SetValue("echo shelltest")
	...
	resMsg := cmd()
	if out, ok := resMsg.(shellOutputMsg); ok {
		if !strings.Contains(string(out), "shelltest") {
			t.Errorf("Expected output 'shelltest', got '%s'", out)
		}
	} else {
		t.Errorf("Expected shellOutputMsg, got %T", resMsg)
	}

	// Should return to ModeChat after execution?
	// The implementation sets ModeChat on receiving shellOutputMsg in Update
	// So we need to update with the result msg
	newM, _ = m.Update(resMsg)
	m = newM.(InteractiveModel)

	if m.mode != ModeChat {
		t.Errorf("Expected to return to ModeChat, got %v", m.mode)
	}
}
...
func TestLoadModelsFromFile(t *testing.T) {
	...
	jsonContent := `{
		"models": [
			{"name": "model1", "displayName": "Model One", "description": "Desc One"},
			{"name": "model2"}
		]
	}`
	...
	if models[0].Name != "Model One" {
		t.Errorf("Expected 'Model One', got '%s'", models[0].Name)
	}
	if models[0].Value != "model1" {
		t.Errorf("Expected value 'model1', got '%s'", models[0].Value)
	}

	// Fallback check
	if models[1].Name != "model2" { // Falls back to Name if DisplayName empty? No, logic says `name := m.DisplayName; if name == "" { name = m.Name }`
