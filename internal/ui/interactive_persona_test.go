...
func TestInteractiveModel_Persona_Selection(t *testing.T) {
	...
	// Inject a known persona for testing
	testID := "test-persona"
	m.personaManager.AddPersona(testID, agent.Persona{
		Name: "Test Persona",
	...
	// Find the test persona index
	var targetIndex int
	found := false
	for i, item := range m.list.Items() {
		if p, ok := item.(PersonaItem); ok && p.Value == testID {
			targetIndex = i
			found = true
			break
		}
	...
	if !found {
		t.Fatal("Could not find injected test persona")
	}
	...
	if m.currentPersona != testID {
		t.Errorf("Expected currentPersona to be '%s', got '%s'", testID, m.currentPersona)
	}
	if m.mode != ModeChat {
...
func TestInteractiveModel_Persona_PromptInjection(t *testing.T) {
	...
	// Inject a specific persona for prompt injection test
	testID := "injector"
	sysPrompt := "You are an INJECTED SYSTEM PROMPT."
	m.personaManager.AddPersona(testID, agent.Persona{
		Name: "Injector",
		SystemPrompt: sysPrompt,
	})

	m.currentPersona = testID
	...
	if !strings.Contains(mockAgent.LastPrompt, sysPrompt) {
		t.Errorf("System prompt injection failed. Expected '%s', got: %s", sysPrompt, mockAgent.LastPrompt)
	}
