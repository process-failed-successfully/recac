...
// MockPersonaManager for testing UI without relying on external config
type MockPersonaManager struct {
	Personas map[string]agent.Persona
}

func (m *MockPersonaManager) LoadPersonas() error { return nil }

func (m *MockPersonaManager) ListSorted() []string {
	var ids []string
...
func (m *MockPersonaManager) GetPersona(id string) (agent.Persona, bool) {
	p, ok := m.Personas[id]
	return p, ok
}
...
func TestInteractiveModel_Persona_Selection(t *testing.T) {
	...
	// Inject Mock Manager with explicit personas
	m.personaManager = &MockPersonaManager{
		Personas: map[string]agent.Persona{
			"default": {Name: "Default", Description: "Default persona"},
			"junior":  {Name: "Junior Dev", Description: "A junior developer"},
		},
	}
...
func TestInteractiveModel_Persona_PromptInjection(t *testing.T) {
	...
	// Inject Mock Manager with specific system prompt
	m.personaManager = &MockPersonaManager{
		Personas: map[string]agent.Persona{
			"teacher": {Name: "The Teacher", SystemPrompt: "You are an expert Computer Science Teacher"},
		},
	}
