...
func TestInteractiveModel_Persona_PromptInjection(t *testing.T) {
	...
	// Execute the command to trigger the goroutine and get the stream message
	msg := cmd()
	...
	// Read from the channel to ensure execution completion (simulating waiting for stream)
	// Since CapturingMockAgent runs immediately, this should be safe.
	<-streamMsg.ChunkChan
