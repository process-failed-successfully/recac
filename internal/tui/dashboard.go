...
func (m DashboardModel) updateMain(msg tea.Msg) (DashboardModel, tea.Cmd) {
	...
	case tea.KeyMsg:
		...
		case "I":
			if len(m.selectedJobs) > 0 {
				m.pendingJobId = "MULTIPLE_I"
				m.pendingAction = "skip multiple"
				m.viewState = viewConfirmation
				return m, nil
			}
			selected := m.table.SelectedRow()
...
func (m DashboardModel) View() string {
	...
	case viewConfirmation:
		...
		} else if m.pendingAction == "promote multiple" {
			dialogMsg = fmt.Sprintf("Are you sure you want to PROMOTE %d selected jobs?\n\n(y/Enter: confirm, n/q/Esc: cancel)", len(m.selectedJobs))
		} else if m.pendingAction == "demote multiple" {
			dialogMsg = fmt.Sprintf("Are you sure you want to DEMOTE %d selected jobs?\n\n(y/Enter: confirm, n/q/Esc: cancel)", len(m.selectedJobs))
		} else if m.pendingAction == "hold multiple" {
...
func skipJobCmd(host, id string) tea.Cmd {
	return func() tea.Msg {
		req, err := http.NewRequest(http.MethodPost, host+"/jobs/"+id+"/skip", nil)
		if err != nil {
			return actionMsg{Err: err}
		}
		resp, err := http.DefaultClient.Do(req)
