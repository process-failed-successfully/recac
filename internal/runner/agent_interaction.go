...
func (s *Session) runQAAgent(ctx context.Context) error {
	...
	// We use s.hasSignal which checks both DB and filesystem (fallback for tests).
	if s.hasSignal("QA_PASSED") {
		s.Logger.Info("QA passed (signal verified)")
		return nil
	}

	// Read raw value to distinguish explicit failure
	var val string
	if s.DBStore != nil {
		val, _ = s.DBStore.GetSignal(s.Project, "QA_PASSED")
	}
	s.Logger.Info("QA result signal check", "signal", val)
