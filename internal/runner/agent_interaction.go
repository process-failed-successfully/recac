...
func (s *Session) runQAAgent(ctx context.Context) error {
	...
	// Mock Mode Bypass: If running in mock mode, accept text confirmation
	if s.AgentProvider == "mock" && strings.Contains(response, "LGTM") {
		s.Logger.Info("QA passed (mock mode text match)")
		return nil
	}
