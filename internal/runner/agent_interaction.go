...
func (s *Session) SelectPrompt() (string, string, bool, error) {
	...
	// Priority 1: User/System forced a specific task
	if s.SelectedTaskID != "" {
		features := s.loadFeatures()
		...
		// Priority 2: Deterministic assignment found a pending feature
		...
		// Priority 3: No forced task, and no pending features found -> All Complete
