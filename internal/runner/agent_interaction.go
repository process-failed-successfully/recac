...
func (s *Session) SelectPrompt() (string, string, bool, error) {
	...
	// 4. Task Assignment Logic (Priority: Forced > Deterministic > All Complete)
	if s.SelectedTaskID != "" {
		features := s.loadFeatures()
		...
		// Deterministic Assignment (Next Pending Feature)
		...
		if len(features) > 0 {
			// All features exist but are done
			...
			// No features at all (and Initializer didn't run or failed)
