...
func initializeLogging(project, rootDir string) *slog.Logger {
	...
	agentsLogsDir := filepath.Join(rootDir, "agents", "logs")
	if err := os.MkdirAll(agentsLogsDir, 0755); err != nil {
		...
		// BUT skip if running tests to avoid race conditions and global state mutation
