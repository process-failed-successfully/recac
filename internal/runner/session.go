...
func initializeLogging(project string) *slog.Logger {
	// Check for explicit logs directory from environment
	agentsLogsDir := os.Getenv("RECAC_LOGS_DIR")

	// If not set, check if we are in test mode
	if agentsLogsDir == "" && flag.Lookup("test.v") != nil {
		// In tests, we skip file logging and directory creation to avoid pollution
		// unless explicitly requested via RECAC_LOGS_DIR
		return telemetry.NewLogger(true, "", false)
	}
	...
	if err := os.MkdirAll(agentsLogsDir, 0755); err != nil {
		...
		// Replaced slashes with underscores to prevent path traversal issues
		sanitizedProject := strings.ReplaceAll(project, "/", "_")
		timestamp := time.Now().Format("20060102-150405.000") // Added millisecond precision
		logFileName := fmt.Sprintf("%s_agent_%s_%s.log", sanitizedProject, sanitizedProject, timestamp)
