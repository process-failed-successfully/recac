...
func NewSession(d DockerClient, a agent.Agent, workspace, image, project, provider, model string, maxAgents int) *Session {
	...
	dbStore, err := initializeDBStore(workspace)
	if err != nil {
		// Critical failure - Fail Fast
		fmt.Fprintf(os.Stderr, "[Session] CRITICAL: Could not connect to database after retries. Exiting.\n")
		os.Exit(1)
	} else {
		// Success (dbStore might be nil if failed but not errored? No, initializeDBStore returns error if failed)
		// But let's check type
		if dbStore != nil {
			// We can't easily get type from interface without casting, but initializeDBStore logs success.
			slog.Info("[DB] Store initialized successfully", "project", project)
		}
	}
...
func initializeDBStore(workspace string) (db.Store, error) {
	...
	if err == nil {
		fmt.Fprintf(os.Stderr, "[Session] DB Store initialized successfully: type=%s\n", storeConfig.Type)
	}
