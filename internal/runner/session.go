...
func NewSession(d DockerClient, a agent.Agent, workspace, image, project, provider, model string, maxAgents int) *Session {
	...
	dbStore, err := initializeDBStore(storeConfig)
...
func NewSessionWithStateFile(d DockerClient, a agent.Agent, workspace, image, project, agentStateFile, provider, model string, maxAgents int) *Session {
	...
	dbStore, err := initializeDBStore(storeConfig)
	if err != nil {
		// Log warning but proceed (consistent with previous behavior of not failing fast in this constructor)
		fmt.Printf("Warning: Failed to initialize DB store (%s) after retries: %v\n", storeConfig.Type, err)
	}
...
func initializeLogging(project string) *slog.Logger {
	...
	// 1. Check for RECAC_LOGS_DIR (used in tests and CI to prevent pollution)
	// 2. Fallback to agents/logs in current working directory (for Promtail/Docker monitoring)
	...
	if err := os.MkdirAll(agentsLogsDir, 0755); err != nil {
		fmt.Printf("Warning: Failed to create agents/logs directory (%s): %v\n", agentsLogsDir, err)
	} else {
...
func initializeDBStore(storeConfig db.StoreConfig) (db.Store, error) {
