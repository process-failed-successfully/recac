...
func initializeLogging(project string) *slog.Logger {
	...
	// Default to host cwd, but respect override for tests/isolation
	var agentsLogsDir string
	if override := os.Getenv("RECAC_LOGS_DIR"); override != "" {
		agentsLogsDir = filepath.Join(override, "agents", "logs")
	} else {
		cwd, _ := os.Getwd()
	...
	if err := os.MkdirAll(agentsLogsDir, 0755); err != nil {
		...
		// Use millisecond precision to prevent collisions in concurrent environments (e.g., tests)
