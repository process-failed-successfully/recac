...
func initializeLogging(project string) *slog.Logger {
	...
	// Tests can override this via RECAC_LOGS_DIR to prevent source tree pollution
	var agentsLogsDir string
	if envDir := os.Getenv("RECAC_LOGS_DIR"); envDir != "" {
		agentsLogsDir = envDir
	} else {
		// Default: Create agents/logs directory in the current working directory (host)
