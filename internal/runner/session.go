...
func (s *Session) RunLoop(ctx context.Context) error {
	...
	for {
		...
		if s.hasSignal("PROJECT_SIGNED_OFF") {
			shouldExit, shouldContinue := s.handleProjectSignOff(ctx)
			if shouldContinue {
				continue
			}
			if shouldExit {
...
// executeCycle runs a single agent iteration cycle.
func (s *Session) executeCycle(ctx context.Context) error {
	// Select appropriate prompt and role
	prompt, role, isManager, err := s.SelectPrompt()
	if err != nil {
		fmt.Printf("Error selecting prompt: %v\n", err)
		// We return an error to break the loop, assuming prompt failure is fatal or terminal
		return err
	}
	...
	if err != nil {
		...
		return nil // Retry loop without tripping no-op breaker
	}
...
// handleProjectSignOff handles the final steps when a project is signed off.
// Returns (shouldExit, shouldContinue) bools.
func (s *Session) handleProjectSignOff(ctx context.Context) (bool, bool) {
	...
	if s.AutoMerge && s.BaseBranch != "" {
		...
		if err != nil {
			...
			if err := gitClient.Checkout(s.Workspace, s.BaseBranch); err != nil {
				...
				// 5. Checkout back to feature branch
...
// handleQualityGates handles QA and Manager review transitions.
// Returns true if a transition occurred and the loop should continue.
func (s *Session) handleQualityGates(ctx context.Context) bool {
	if s.hasSignal("QA_PASSED") {
		...
		if err := s.runManagerAgent(ctx); err != nil {
			...
			return true // Next iteration will run Cleaner
		}
	}
	...
	if s.hasSignal("COMPLETED") {
		...
		if err := s.runQAAgent(ctx); err != nil {
			...
			return true // Next iteration will run Manager
		}
	}
