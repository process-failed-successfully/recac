...
type Session struct {
	...
	HomeDir                   string              // Override for user home directory (for testing)
	StatFunc                  func(string) (os.FileInfo, error) // Function for checking file existence (mockable)
...
func (s *Session) Start(ctx context.Context) error {
	...
	if homeDir != "" {
		...
		// Use injectable StatFunc if available, otherwise os.Stat
		stat := s.StatFunc
		if stat == nil {
			stat = os.Stat
		}
