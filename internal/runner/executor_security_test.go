...
func TestProcessResponse_Security(t *testing.T) {
	...
	// 3. Safe Cleanup (rm -rf *) - This previously failed (False Positive)
	respClean := "I will clean up.\n```bash\nrm -rf *\n```"
	outClean, err := s.ProcessResponse(context.Background(), respClean)
	if err != nil {
		t.Fatalf("ProcessResponse failed: %v", err)
	}

	if strings.Contains(outClean, "[BLOCKED]") {
		t.Errorf("Cleanup command (rm -rf *) was blocked! %s", outClean)
	}
