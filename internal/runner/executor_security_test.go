...
func TestProcessResponse_Security(t *testing.T) {
	...
	if s.Scanner == nil {
		t.Fatal("Scanner is nil in test setup!")
	}
	...
	// 3. Dangerous Command in 'sh' block
	respSh := "I will delete everything.\n```sh\nrm -rf /\n```"
	outSh, err := s.ProcessResponse(context.Background(), respSh)
	if err != nil {
		t.Fatalf("ProcessResponse failed: %v", err)
	}

	if !strings.Contains(outSh, "[BLOCKED]") {
		t.Errorf("Dangerous 'sh' command was NOT blocked (regex missed it?) output: %s", outSh)
	}
}
