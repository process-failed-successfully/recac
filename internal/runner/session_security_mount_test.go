...
func TestSession_SensitiveMounts_ReadOnly(t *testing.T) {
	...
	for _, p := range sensitivePaths {
		...
		if err := os.MkdirAll(path, 0755); err != nil {
			t.Fatalf("Failed to create dummy sensitive dir %s: %v", path, err)
		}
	}
	...
	for _, path := range sensitivePaths {
		...
		for _, bind := range capturedBinds {
			// Check if bind starts with the sensitive path (or contains it)
			// But since we control HOME, we can be more specific:
			expectedBindPrefix := filepath.Join(tempHome, path)
	...
	if !foundAny {
		t.Fatal("No sensitive paths were found in binds, but they were created in temp HOME.")
	}
}
