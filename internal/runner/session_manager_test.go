...
func TestRemoveSession_Error(t *testing.T) {
	...
	// Create a dummy file to verify permissions on THIS directory
	dummyPath := filepath.Join(sm.sessionsDir, "dummy_check")
	err = os.WriteFile(dummyPath, []byte("check"), 0644)
	require.NoError(t, err)
	...
	// Verify permissions are actually enforced on this directory
	if err := os.Remove(dummyPath); err == nil {
		t.Skip("Skipping test: filesystem permissions are not enforced on sessions dir (running as root?)")
	}
