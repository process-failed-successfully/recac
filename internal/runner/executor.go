...
func (s *Session) checkBlockers(ctx context.Context) error {
	...
	// Check local files if UseLocalAgent is true, OR docker files if Docker is present
	...
	for _, bf := range blockerFiles {
		...
		if s.UseLocalAgent {
			...
			content, execErr := s.Docker.Exec(ctx, s.GetContainerID(), checkCmd)
			if execErr == nil && len(strings.TrimSpace(content)) > 0 {
		...
		if fileExists && len(trimmed) > 0 {
			...
			// Optional: log error if necessary, but skipping for compatibility with old behavior
		}
	}
	return nil
}
