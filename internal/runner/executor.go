...
func (s *Session) ProcessResponse(ctx context.Context, response string) (string, error) {
	...
	if err := s.checkBlockers(ctx); err != nil {
		// FIXED: Return accumulated output even if blocked, so agent/user knows what happened before blocking.
...
func (s *Session) checkBlockers(ctx context.Context) error {
	...
	// Check for blocker files in Docker OR Local
	...
	for _, bf := range blockerFiles {
		...
		if s.UseLocalAgent {
			...
			if _, statErr := os.Stat(path); statErr == nil {
				...
				err = statErr // Treat not exist as error for now (skip)
			}
