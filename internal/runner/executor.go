...
func (s *Session) checkBlockers(ctx context.Context) error {
	...
	// Helper to check for false positives
	isFalsePositive := func(content string) bool {
		cleanStr := strings.ToLower(strings.TrimSpace(content))
	...
	if s.UseLocalAgent {
		...
		for _, bf := range blockerFiles {
			...
			if err == nil && len(content) > 0 {
				...
				if isFalsePositive(strContent) {
					s.Logger.Info("ignoring false positive blocker (local)", "file", bf, "content", strContent)
					os.Remove(path)
					continue
				}

				s.Logger.Warn("agent reported blocker file (local)", "file", bf)
				s.Logger.Warn("blocker content", "content", strContent)
