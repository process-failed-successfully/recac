...
func (s *Session) executeCommandBlock(ctx context.Context, cmdScript string, index, total int) (string, error) {
	...
	if s.UseLocalAgent {
		...
		cmd.Env = append(os.Environ(), fmt.Sprintf("RECAC_PROJECT_ID=%s", s.Project))
			...
			// Inject DB config if missing from env (critical for agent-bridge)
			if os.Getenv("RECAC_DB_URL") == "" {
				cmd.Env = append(cmd.Env, fmt.Sprintf("RECAC_DB_URL=%s", filepath.Join(s.Workspace, ".recac.db")))
			}
			if os.Getenv("RECAC_DB_TYPE") == "" {
				cmd.Env = append(cmd.Env, "RECAC_DB_TYPE=sqlite")
			}
