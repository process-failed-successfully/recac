...
func (s *Session) checkNoOpBreaker(executionOutput string) error {
	if executionOutput == "" {
		s.NoOpCount++
		if s.NoOpCount >= 3 {
			return fmt.Errorf("%w: Agent has produced 3 consecutive responses with no commands", ErrNoOp)
		}
	} else {
...
func (s *Session) checkStalledBreaker(role string, passingCount int) error {
	...
	if s.StalledCount >= s.ManagerFrequency*3 {
		return fmt.Errorf("%w: Stalled for %d iterations", ErrStalled, s.StalledCount)
	}
