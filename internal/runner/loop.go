...
func (s *Session) RunLoop(ctx context.Context) error {
	...
	// PLAN ONLY MODE
	if s.PlanOnly {
		s.Logger.Info("Plan Only Mode detected. Generating plan...")
		specContent, err := s.ReadSpec()
		if err != nil {
			return fmt.Errorf("failed to read spec for planning: %w", err)
		}

		plan, err := s.GeneratePlanOnly(ctx, specContent)
		if err != nil {
			return err
		}
		...
		s.Notifier.Notify(ctx, notify.EventSuccess, fmt.Sprintf("Project %s: Plan Generated (PLAN.md)", s.Project), s.GetSlackThreadTS())
		return nil
	}
