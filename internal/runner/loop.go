...
func (s *Session) RunLoop(ctx context.Context) error {
	...
	for {
		...
		if s.hasSignal("PROJECT_SIGNED_OFF") {
			...
			if err := s.runAutoMerge(ctx); err != nil {
				s.Logger.Warn("auto-merge logic encountered issues", "error", err)
				// We don't necessarily want to fail the whole session if auto-merge fails,
				// or maybe we do. For now, we log and proceed to cleanup.
			}
...
// handleMergeGuardrail checks for upstream conflicts. Returns true if it's safe to proceed (no conflicts or resolved), false if conflicts exist or issues found.
func (s *Session) handleMergeGuardrail(ctx context.Context) bool {
...
// runAutoMerge handles the auto-merge logic.
func (s *Session) runAutoMerge(ctx context.Context) error {
	// Auto-Merge Logic
