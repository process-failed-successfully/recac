...
func TestSession_RunQAAgent_SignalSuccess(t *testing.T) {
	...
	mockAgent.SideEffect = func() {
		...
		if err := dbStore.SetSignal("test-project", "QA_PASSED", "true"); err != nil {
			panic(err)
		}
	}
...
func TestSession_RunManagerAgent_SignalSuccess(t *testing.T) {
	...
	// Set signal
	if err := dbStore.SetSignal("test-project", "PROJECT_SIGNED_OFF", "true"); err != nil {
...
func TestSessionManager_Coverage(t *testing.T) {
	...
	if err := os.WriteFile(sessionFile2, []byte(`{"name":"session-2", "pid":999999, "status":"running"}`), 0644); err != nil {
		t.Fatalf("WriteFile failed: %v", err)
	}
