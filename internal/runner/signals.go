...
func (s *Session) hasSignal(name string) bool {
	...
	if _, err := os.Stat(path); err == nil {
		...
		// UNLESS we are in mock/test mode
		...
		if privilegedSignals[name] {
			// Allow privileged signals from filesystem if using mock agent (tests) or if DB is missing
			if s.AgentProvider == "mock" || s.DBStore == nil {
				s.Logger.Warn("allowing filesystem-based privileged signal (test/mock mode)", "signal", name)
			} else {
...
func (s *Session) createSignal(name string) error {
	if s.DBStore == nil {
		// Fallback to filesystem if DB is missing (e.g. tests)
		path := filepath.Join(s.Workspace, name)
		if err := os.WriteFile(path, []byte("true"), 0644); err != nil {
			return fmt.Errorf("failed to create signal file: %w", err)
		}
		s.Logger.Info("created signal (filesystem fallback)", "signal", name)
		return nil
	}
	if err := s.DBStore.SetSignal(s.Project, name, "true"); err != nil {
		return err
