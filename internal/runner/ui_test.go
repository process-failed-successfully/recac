...
func TestSession_RunLoop_UIVerification(t *testing.T) {
	...
	s := &Session{
		...
		ManagerFrequency: 5,
		MaxIterations:    5, // Fix: Limit iterations to prevent test timeout
	...
	// ErrNoOp is expected if the agent returns empty responses (legacy).
	// ErrMaxIterations is expected if the agent keeps responding (new fallback behavior) but signals aren't processed.
