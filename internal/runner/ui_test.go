...
func TestSession_RunLoop_UIVerification(t *testing.T) {
	...
	s := &Session{StatFunc: os.Stat,
		...
		ManagerFrequency: 5,
		MaxIterations:    2, // FORCE EXIT to prevent infinite loop
		Notifier:         notify.NewManager(func(string, ...interface{}) {}),
		Logger:           telemetry.NewLogger(true, "", false),
		SleepFunc:        func(d time.Duration) {}, // No sleep in tests
	}
	...
	// ErrMaxIterations is expected because the MockAgent returns empty responses and we force exit.
	if err != nil && !errors.Is(err, ErrMaxIterations) && !errors.Is(err, ErrNoOp) {
