...
func TestSession_RunLoop_UIVerification(t *testing.T) {
	...
	s := &Session{
		...
		ManagerFrequency: 5,
		MaxIterations:    5, // LIMIT ITERATIONS to prevent infinite loops/timeout
		DBStore:          mockDB, // Inject Mock DB
		Notifier:         notify.NewManager(func(string, ...interface{}) {}),
		Logger:           telemetry.NewLogger(true, "", false),
		SleepFunc:        func(time.Duration) {}, // Mock sleep to speed up test
	}
