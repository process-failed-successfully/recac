...
func TestSession_RunLoop_UIVerification(t *testing.T) {
	...
	s := &Session{
		...
		ManagerFrequency: 5,
		MaxIterations:    5, // Prevent infinite loop if MockAgent logic doesn't trigger exit
	...
	// ErrMaxIterations is also acceptable as we limited it to 5 to prevent timeouts.
