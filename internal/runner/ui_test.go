...
func TestSession_RunLoop_UIVerification(t *testing.T) {
	...
	s := &Session{StatFunc: os.Stat,
		...
		DBStore:          store, // Add DBStore
		MaxIterations:    5,     // Limit iterations to prevent hanging
	}

	// 7. Capture Stdout? (Hard to do in test without refactor).
	...
	// ErrMaxIterations might be returned if signals don't work as expected, but with DBStore it should work.
