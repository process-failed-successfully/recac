...
func TestSession_RunLoop_UIVerification(t *testing.T) {
	...
	s := &Session{
		...
		ManagerFrequency: 5,
		MaxIterations:    5, // Limit iterations to avoid timeout if completion is delayed
	...
	// ErrMaxIterations is also acceptable if completion signal isn't processed in time.
