...
func TestGetPrompt_Overrides(t *testing.T) {
	...
	fakeCwd, err := os.MkdirTemp("", "recac-cwd-test")
	if err != nil {
		t.Fatalf("Failed to create temp fake cwd: %v", err)
	}
	defer os.RemoveAll(fakeCwd)

	// Create .recac/prompts in fakeCwd
	localRecacDir := filepath.Join(fakeCwd, ".recac", "prompts")
