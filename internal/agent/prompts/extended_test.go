...
func TestGetPrompt_Overrides(t *testing.T) {
	...
	// Disable env override to fallback to local check
	t.Setenv("RECAC_PROMPTS_DIR", "")

	// Use a temp directory to simulate CWD to avoid writing to source tree
	cwdSim := t.TempDir()
	t.Chdir(cwdSim)

	localRecacDir := filepath.Join(cwdSim, ".recac", "prompts")
