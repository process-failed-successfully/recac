...
func TestGetPrompt_Overrides(t *testing.T) {
	...
	// We run this in a subtest or block where we change CWD safely.
	// We also need to unset RECAC_PROMPTS_DIR.
	// Since t.Setenv in step 2 persists, we override it here.

	func() {
		// Capture current CWD to restore later
		oldCwd, err := os.Getwd()
		if err != nil {
			t.Fatalf("Failed to get cwd: %v", err)
		}

		// Create a temp dir for CWD simulation
		tmpCwd := t.TempDir()
		...
		// Unset the env var for this block (manually or via Setenv if we were in a subtest)
		// Since we are in the same test function, t.Setenv("RECAC_PROMPTS_DIR", "") works for the rest of the function.
