...
func TestGetPrompt_Overrides(t *testing.T) {
	...
	t.Run("LocalOverride", func(t *testing.T) {
		// Mock getwd instead of changing CWD
		originalGetwd := getwd
		...
		// Create .recac/prompts in tmpDir (which mimics CWD)
	...
	t.Run("GlobalOverride", func(t *testing.T) {
		...
		// Ensure local override doesn't interfere (mock getwd to empty dir)
		originalGetwd := getwd
		defer func() { getwd = originalGetwd }()
		emptyDir := t.TempDir()
		getwd = func() (string, error) { return emptyDir, nil }

		// Create .recac/prompts in tmpDir (Home)
