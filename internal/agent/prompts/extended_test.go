...
func TestGetPrompt_Overrides(t *testing.T) {
	...
	// Use a separate temp dir and chdir into it to act as CWD
	// This prevents writing to the actual source tree.
	cwdDir := t.TempDir()
	...
	// Restore original WD at the end of the test
	defer func() {
		if err := os.Chdir(originalWd); err != nil {
			// In a real scenario, this might be fatal, but in a test panic is acceptable/caught
	...
	// Because we changed CWD, GetPrompt should find the file in CWD/.recac/prompts
	content, err = GetPrompt(promptName, nil)
	if err != nil {
