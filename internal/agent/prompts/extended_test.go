...
func TestGetPrompt_Overrides(t *testing.T) {
	...
	// We need to simulate running in a directory with .recac/prompts.
	// We should NOT write to the actual CWD (source tree).
	// Instead, we switch CWD to a temp directory.

	oldCwd, err := os.Getwd()
	if err != nil {
		t.Fatalf("Failed to get current CWD: %v", err)
	}

	testCwd := t.TempDir()
	if err := os.Chdir(testCwd); err != nil {
		t.Fatalf("Failed to change CWD to temp dir: %v", err)
	}
	defer func() {
		// Restore CWD
	...
	// Create .recac/prompts in testCwd
	localRecacDir := filepath.Join(testCwd, ".recac", "prompts")
