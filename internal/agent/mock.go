...
func (m *MockAgent) Send(ctx context.Context, prompt string) (string, error) {
	...
	// Updated to be more specific to avoid conflict with Planning Phase
	if strings.Contains(promptLower, "qa report") || (strings.Contains(promptLower, "## your role - project manager") && strings.Contains(promptLower, "qa report")) {
		return "Based on the QA Report, I approve the project.\n```bash\nagent-bridge signal PROJECT_SIGNED_OFF true --privileged\n```", nil
	}

	// 2. Project Manager / Planning Phase (Ticket Generation)
	// Precedes Coding Phase to ensure "Prime" keyword in spec doesn't trigger coding response
	// Updated to remove generic "project manager" check which belongs to Manager Review
