...
func (m *MockAgent) Send(ctx context.Context, prompt string) (string, error) {
	...
	// 1. Completion Check (Feedback Loop)
	if strings.Contains(prompt, "primes.json created successfully") {
		return "Task completed. Implementation verified.", nil
	}
		...
		return `[
  {
	...
	if strings.Contains(prompt, "PRIMES") || strings.Contains(prompt, "primes.py") {
		return `
Here is the implementation for calculating prime numbers.
...
if [ -f primes.json ]; then
    echo "primes.json created successfully."
else
    echo "Error: primes.json not found."
    exit 1
fi
` + "```" + `
