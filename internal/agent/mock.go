...
func (m *MockAgent) Send(ctx context.Context, prompt string) (string, error) {
	...
	// Condition 1: Strong Feature Match (ignores role if feature ID is explicit)
	isPrimesFeature := strings.Contains(prompt, "req-script-prints-primes-up-to-100")

	// Condition 2: Keyword + Role Match (fallback for cases without exact ID)
	hasPrimesKeyword := strings.Contains(prompt, "[PRIMES]") || strings.Contains(prompt, "primes.py") || strings.Contains(prompt, "Script prints primes")
	hasCodingRole := strings.Contains(prompt, "CODING AGENT") || strings.Contains(prompt, "Developer") || strings.Contains(prompt, "Agent") || strings.Contains(prompt, "primes.py")

	if (isPrimesFeature || (hasPrimesKeyword && hasCodingRole)) &&
