...
func (m *MockAgent) Send(ctx context.Context, prompt string) (string, error) {
	...
	// 1. Check for Prime Python Scenario - Planning Phase
	// Trigger: The specific instruction from AppSpec in prime-python.go
		...
		return `[
  {
	...
	// 2. Check for Prime Python Scenario - Implementation Phase
	// Trigger: The ticket description which contains specific instructions
	if strings.Contains(prompt, "Create a python script named 'primes.py'") {
		return `I will create the primes.py script and generates the output.
