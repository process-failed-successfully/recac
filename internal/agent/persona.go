...
// PersonaManager manages available personas.
type PersonaManager struct {
	personas map[string]Persona
}

// DefaultPersonas defines the built-in personas.
...
// LoadPersonas loads additional personas from ~/.recac/personas.yaml if it exists.
func (pm *PersonaManager) LoadPersonas() error {
	home, err := os.UserHomeDir()
	if err != nil {
		return err
	}
	path := getPersonasFilePath(home)

	// Check if file exists
	if _, err := os.Stat(path); os.IsNotExist(err) {
		return nil // No custom personas, that's fine
	}
...
func getPersonasFilePath(home string) string {
...
// AddPersona adds a persona programmatically.
