...
func (c *BaseClient) SendWithRetry(ctx context.Context, prompt string, sendOnce func(context.Context, string) (string, error)) (string, error) {
	...
	maxRetries := 5 // Increased from 3 to handle 429s better
	var lastErr error

	for i := 0; i <= maxRetries; i++ {
		if i > 0 {
			waitTime := c.BackoffFn(i)
			// Heuristic: If 429 (Rate Limit), wait longer (at least 10s)
			if lastErr != nil && strings.Contains(lastErr.Error(), "429") {
				telemetry.LogInfo("Rate limit detected (429), increasing backoff", "project", c.Project)
				if waitTime < 10*time.Second {
					waitTime = 10*time.Second + waitTime
				}
			}
...
func (c *BaseClient) SendStreamWithRetry(ctx context.Context, prompt string, sendStreamOnce func(context.Context, string, func(string)) (string, error), onChunk func(string)) (string, error) {
	...
	for i := 0; i <= maxRetries; i++ {
		if i > 0 {
			waitTime := c.BackoffFn(i)
			// Heuristic: If 429 (Rate Limit), wait longer
			if lastErr != nil && strings.Contains(lastErr.Error(), "429") {
				telemetry.LogInfo("Rate limit detected (429), increasing backoff", "project", c.Project)
				if waitTime < 10*time.Second {
					waitTime = 10*time.Second + waitTime
