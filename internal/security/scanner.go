...
var (
	...
	reDangerousCmd    = regexp.MustCompile(`(?i)\b(rm|cat|cp|mv|chmod|chown)\b.*?[\s/<>\"'\\](\.[s]sh|\.[a]ws|\.[c]onfig|\.[g]emini|/etc/pass[w]d|/etc/shado[w]|\.[e]nv|\.git-credential[s]|\.[n]etrc|/proc/self/enviro[n])`)
