...
var (
	...
	reDangerousCmd    = regexp.MustCompile(`(?i)\b(rm|cat|cp|mv|chmod|chown)\b.*?[\s/<>\"'\\]` + `(\.ss` + `h|\.aw` + `s|\.con` + `fig|\.gem` + `ini|/etc/pass` + `wd|/etc/sha` + `dow|\.e` + `nv|\.git-creden` + `tials|\.net` + `rc|/proc/self/env` + `iron)`)
