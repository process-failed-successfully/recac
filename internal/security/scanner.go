...
var (
	...
	// reDangerousCmd matches dangerous commands (rm, cat, etc.) targeting sensitive files.
	// We use bracket injection (e.g. [e]nv) to avoid self-matching.
	// We require a boundary (space, slash, brackets, quotes) to avoid matching unrelated files like "my.config".
