...
var (
	...
	reDangerousCmd    = regexp.MustCompile(`(?i)\b(rm|cat|cp|mv|chmod|chown|grep|sed|awk|curl|wget|tee)\b.*(\.ssh|\.aws|\.config|\.gemini|/etc/passwd|/etc/shadow|\.env|\.kube|\.docker|\.git-credentials|\.npmrc)`)
