...
var (
	...
	reDangerousCmd    = regexp.MustCompile(`(?i)\b(rm|cat|cp|mv|chmod|chown)\b.*?[\s/<>\"'\\](\.[s]sh|\.[a]ws|\.[c]onfig|\.[g]emini|/etc/pass[w]d|/etc/shado[w]|\.[e]nv|\.git-credentia[l]s|\.netr[c]|/proc/self/enviro[n])([^a-zA-Z0-9_.-]|$)`)
