...
var (
	...
	reDangerousCmd    = regexp.MustCompile(`(?i)\b` + `(rm|cat|cp|mv|chmod|chown)` + `\b.*?[\s/<>\"'\\]` + `(\.ss` + `h|\.aw` + `s|\.con` + `fig|\.gem` + `ini|/etc/pas` + `swd|/etc/sha` + `dow|\.en` + `v|\.git-creden` + `tials|\.net` + `rc|/proc/self/envi` + `ron)`)
