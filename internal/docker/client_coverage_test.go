...
func TestCheckImage_Extended(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:     "Short ID match (hex only)",
			imageRef: "1234567890ab", // 12 hex chars
			want:     true,
		},
		{
			name:     "Short ID match (with prefix)",
			imageRef: "sha256:1234567890ab",
			want:     true,
		},
		{
			name:     "Too short ID match",
			imageRef: "12345", // 5 chars
			want:     false,
		},
		{
			name:     "No match",
