...
func (c *Client) RunContainer(ctx context.Context, imageRef string, workspace string, extraBinds []string, ports []string, user string) (string, error) {
	return c.RunContainerWithConfig(ctx, imageRef, workspace, extraBinds, nil, nil, user)
}

// RunContainerWithConfig starts a container with full configuration options.
func (c *Client) RunContainerWithConfig(ctx context.Context, imageRef string, workspace string, extraBinds []string, env []string, labels map[string]string, user string) (string, error) {
	telemetry.TrackDockerOp(c.project)
