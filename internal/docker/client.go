...
func (c *Client) RunContainer(ctx context.Context, imageRef string, workspace string, extraBinds []string, ports []string, user string) (string, error) {
	return c.RunContainerWithLabels(ctx, imageRef, workspace, extraBinds, ports, nil, user)
}

// RunContainerWithLabels starts a container with the specified image, workspace, env vars, and labels.
// It returns the container ID or an error.
