...
## 2026-07-23 - [Mobile Layout Spacing and Touch Targets]
**Learning:** Found that excessive container padding on mobile viewports (e.g., 20px on both sides) severely limits horizontal screen real estate for data-dense tables. Additionally, horizontally aligned headers with condensed action buttons become difficult touch targets on small screens.
**Action:** When implementing responsive mobile layouts (e.g., `@media (max-width: 768px)`), explicitly reduce `body` and container padding (e.g., to 10px) to maximize usable width. Furthermore, vertically stack header content (`flex-direction: column; align-items: flex-start`) and spread action button groups across the full width (`width: 100%; justify-content: space-between`) to improve touch accessibility.
