...
## 2026-09-24 - Secure environment variable propagation for RECAC prefix
**Vulnerability:** The orchestrator was blindly propagating all host environment variables starting with the `RECAC_` prefix to the untrusted agent container via `os.Environ()` in `internal/runner/session.go`. Since sensitive secrets (like `RECAC_DB_URL`, `RECAC_GITHUB_TOKEN`, `RECAC_JIRA_WEBHOOK_SECRET`) share this prefix with safe configuration variables (like `RECAC_PROVIDER`, `RECAC_MODEL`), this wildcard approach risked leaking critical orchestrator secrets to the agent.
**Learning:** Using a shared prefix for both public configuration and private secrets makes prefix-based environment allowlists inherently insecure.
**Prevention:** When propagating environment variables to untrusted containers, do not use the `RECAC_` prefix as a wildcard allowlist. Instead, use an exact-match list (e.g., `"RECAC_PROVIDER", "RECAC_MODEL", "RECAC_MAX_ITERATIONS"`) to explicitly permit only safe, non-sensitive configurations.
