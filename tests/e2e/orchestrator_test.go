...
func TestOrchestrator_Poller_E2E(t *testing.T) {
	...
	// 5. Claim
	t.Log("Claiming work...")
	// Claiming is done by updating status to In Progress
	if err := poller.UpdateStatus(ctx, item, "In Progress", "Agent claimed ticket"); err != nil {
...
func TestOrchestrator_FullFlow_E2E(t *testing.T) {
	...
	// Create temp SessionManager
	smDir := t.TempDir()
	sm, err := runner.NewSessionManagerWithDir(smDir)
	if err != nil {
