...
func TestOrchestrator_FullFlow_E2E(t *testing.T) {
	...
	sm, err := runner.NewSessionManagerWithDir(filepath.Join(os.TempDir(), "recac-sessions"))
	if err != nil {
		t.Fatalf("Failed to create session manager: %v", err)
	}
	spawner := orchestrator.NewDockerSpawner(logger, dClient, "recac-agent:e2e", "e2e", poller, provider, model, sm)
