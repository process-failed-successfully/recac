...
func TestOrchestrator_Poller_E2E(t *testing.T) {
	...
	items, err := poller.Poll(ctx, slog.New(slog.NewTextHandler(os.Stdout, nil)))
	if err != nil {
	...
	// 5. Claim (Update Status to In Progress)
	t.Log("Claiming work...")
	if err := poller.UpdateStatus(ctx, item, "In Progress", "Claiming ticket for E2E test"); err != nil {
