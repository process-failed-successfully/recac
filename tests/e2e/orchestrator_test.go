...
func TestOrchestrator_Poller_E2E(t *testing.T) {
	...
	// 5. Claim
	t.Log("Claiming work...")
	if err := poller.UpdateStatus(ctx, item, "In Progress", "Claimed by Orchestrator"); err != nil {
...
func TestOrchestrator_FullFlow_E2E(t *testing.T) {
	...
	spawner := orchestrator.NewDockerSpawner(logger, dClient, "recac-agent:e2e", projectKey, poller, provider, model, sm)
