...
func TestOrchestrator_Poller_E2E(t *testing.T) {
	...
	// Pass nil logger as we don't need detailed logging for this manual poll
	...
	// 5. Claim
	t.Log("Claiming work...")
	// Use UpdateStatus to claim (set to In Progress)
	if err := poller.UpdateStatus(ctx, item, "In Progress", "Claiming work (E2E Test)"); err != nil {
...
func TestOrchestrator_FullFlow_E2E(t *testing.T) {
	...
	// Create Session Manager for Spawner
	sessionsDir := filepath.Join(os.TempDir(), fmt.Sprintf("recac-e2e-sessions-%s", timestamp))
	...
	// NewDockerSpawner requires: logger, client, image, projectName, poller, provider, model, sessionManager
