...
func TestPresentation(t *testing.T) {
	...
	t.Run("Generate Presentation Successfully", func(t *testing.T) {
		root, _, _ := newRootCmd()
		// Ensure command is added (in case other tests messed with rootCmd)
		if cmd, _, _ := root.Find([]string{"presentation"}); cmd != presentationCmd {
			root.AddCommand(presentationCmd)
		}
