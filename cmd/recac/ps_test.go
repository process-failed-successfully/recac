...
func TestPsCommandStatusFilter(t *testing.T) {
	tempDir := t.TempDir()
	sessionsDir := filepath.Join(tempDir, "sessions")
	require.NoError(t, os.Mkdir(sessionsDir, 0755))
	...
	// Create mock sessions with different statuses
	sessions := []*runner.SessionState{
		{Name: "session-running", Status: "running", StartTime: time.Now()},
		{Name: "session-completed", Status: "completed", StartTime: time.Now()},
		{Name: "session-error", Status: "error", StartTime: time.Now()},
	}
	...
	t.Run("filter by non-existent status", func(t *testing.T) {
		output, err := executeCommand(rootCmd, "ps", "--status", "non-existent")
		require.NoError(t, err)
