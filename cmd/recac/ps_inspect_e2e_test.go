...
func TestPsCmd_InspectE2E(t *testing.T) {
	...
	sessionName := "test-inspect-session-e2e"
	...
	require.Contains(t, output, "Session Details for 'test-inspect-session-e2e'")
	require.Regexp(t, regexp.MustCompile(`Status:\s+COMPLETED`), output)
	require.Regexp(t, regexp.MustCompile(`Model:\s+test-model`), output)
	require.Regexp(t, regexp.MustCompile(`Total Tokens:\s+300`), output)
