...
func runSetup(cmd *cobra.Command, args []string) error {
	...
	if len(linesToAppend) > 0 {
		...
		// Parse existing env using godotenv for robust key checking
		...
		if err != nil {
			...
			// Wrap close in anon function to ensure it runs even if panic/return happens inside (though unlikely here)
			// Actually standard defer is fine, but let's be safe.
			defer func() {
			...
			for _, line := range linesToAppend {
				...
				// Check if key exists in parsed map
