	...
	if len(linesToAppend) > 0 {
		...
		if err != nil {
			...
			for _, line := range linesToAppend {
				...
				key := parts[0]
				// Check if the key already exists in .env. We prepend \n to both strings
				// to ensure we match "KEY=" at the start of a line, avoiding partial matches
				// (e.g., matching "KEY=" inside "OTHER_KEY=").
				if !strings.Contains("\n"+existingEnvStr, "\n"+key+"=") {
			...
			if contentToAppend != "" {
				if _, err := f.WriteString(contentToAppend); err != nil {
					...
					fmt.Println("Saved secrets to .env file.")
				}
			}
		}
	}
