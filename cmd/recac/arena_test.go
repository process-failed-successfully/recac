...
func TestArenaCmd(t *testing.T) {
	...
	t.Run("Run Arena Success", func(t *testing.T) {
		// Restore factory after this subtest
		currentFactory := agentClientFactory
		defer func() {
			agentClientFactory = currentFactory
		}()
	...
	t.Run("Run Arena Validation Error", func(t *testing.T) {
		// No need to override factory here, but if we did, we should restore it.
		// Since "Run Arena Success" now restores it, this test starts clean (using originalFactory from TestArenaCmd).
		// However, runArena still calls agentClientFactory? No, validation happens before that.
