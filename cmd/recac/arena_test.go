...
func TestArenaCmd(t *testing.T) {
	...
	t.Run("Run Arena Success", func(t *testing.T) {
		...
		// Set flags using Set to behave like CLI parsing and update bound variables if needed
		...
		// Ensure we clean up flags after test to avoid pollution
		defer func() {
	...
	t.Run("Run Arena Validation Error", func(t *testing.T) {
		...
		defer arenaCmd.Flags().Set("competitors", "")
