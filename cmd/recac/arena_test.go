...
func TestArenaCmd(t *testing.T) {
	...
	t.Run("Run Arena Success", func(t *testing.T) {
		...
		// Set flags via Set
		_ = arenaCmd.Flags().Set("competitors", "openai:gpt-4, gemini:gemini-pro")
		_ = arenaCmd.Flags().Set("task", "Who are you?")
		_ = arenaCmd.Flags().Set("file", "")
		_ = arenaCmd.Flags().Set("judge-provider", "mock")
		_ = arenaCmd.Flags().Set("judge-model", "judge")
	...
	t.Run("Run Arena Validation Error", func(t *testing.T) {
		_ = arenaCmd.Flags().Set("competitors", "openai:gpt-4") // Only one
		err := runArena(arenaCmd, []string{})
		assert.Error(t, err)
    ...
    t.Run("Run Arena With File Context", func(t *testing.T) {
        ...
        _ = arenaCmd.Flags().Set("competitors", "openai:gpt-4, gemini:gemini-pro")
        _ = arenaCmd.Flags().Set("task", "Read file")
        _ = arenaCmd.Flags().Set("file", tmpFile)
        _ = arenaCmd.Flags().Set("judge-provider", "mock")
        _ = arenaCmd.Flags().Set("judge-model", "judge")
