...
Example:
  ...
  recac debug "npm run build"`,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if _, err := fmt.Fprintf(cmd.OutOrStdout(), "Running: %s\n", commandStr); err != nil {
			return err
		}
		...
		if _, err := fmt.Fprintln(cmd.OutOrStdout(), "🤖 Analyzing failure with AI..."); err != nil {
			return err
		}
		...
		if scanErr != nil {
			_, _ = fmt.Fprintf(cmd.ErrOrStderr(), "Warning: failed to scan files: %v\n", scanErr)
		}
