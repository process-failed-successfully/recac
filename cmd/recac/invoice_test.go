...
func TestInvoiceCmd(t *testing.T) {
	...
	mockGit.LogFunc = func(dir string, args ...string) ([]string, error) {
		...
		// Use -48 hours to guarantee it's on a different day regardless of timezone or execution time boundaries
		ts3 := now.Add(-48 * time.Hour).Format(time.RFC3339) // 2 days ago (new session)
