...
func TestInvoiceCmd(t *testing.T) {
	...
	// Mock nowFunc to return a fixed time
	// Use a fixed date to avoid flaky tests due to midnight transitions
	fixedNow := time.Date(2023, 10, 27, 12, 0, 0, 0, time.UTC)
	...
	mockGit.LogFunc = func(dir string, args ...string) ([]string, error) {
		...
		now := fixedNow // Use same fixedNow
	...
	// Assertions
	// Use fixedNow for expectedDue
	expectedDue := fixedNow.AddDate(0, 0, 30).Format("Jan 02, 2006")
