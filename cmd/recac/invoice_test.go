...
func TestInvoiceCmd(t *testing.T) {
	...
	mockGit.LogFunc = func(dir string, args ...string) ([]string, error) {
		...
		// Use 50 hours ago to guarantee it falls on a different day even if running near midnight
		ts3 := now.Add(-50 * time.Hour).Format(time.RFC3339) // 2 days ago (new session)
