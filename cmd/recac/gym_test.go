...
func TestRunGymSession(t *testing.T) {
	...
	gymSessionFactory = func(d runner.DockerClient, a agent.Agent, workspace, image, project, provider, model string, maxAgents int) *runner.Session {
		return &runner.Session{
			...
			Project:       project,
			MaxIterations: 1,
			SpecFile:      "app_spec.txt", // Required to prevent ReadSpec error
	...
	// Agent interaction expectation (must be called since loop runs)
	mockAgent.On("Send", mock.Anything, mock.Anything).Return("I'll do it", nil)
