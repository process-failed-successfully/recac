...
type agentFactoryFunc func(context.Context, string, string, string, string) (agent.Agent, error)
type contextKey string

const agentFactoryKey contextKey = "agentFactory"
...
func runArena(cmd *cobra.Command, args []string) error {
	...
	// Resolve agent factory (Context injection > Global)
	getAgent := agentClientFactory
	if v := ctx.Value(agentFactoryKey); v != nil {
		if f, ok := v.(agentFactoryFunc); ok {
			getAgent = f
		}
	}
	...
	for i, compStr := range cleanCompetitors {
		...
		go func(index int, cStr string) {
			...
			ag, err := getAgent(ctx, provider, model, cwd, projName)
			if err != nil {
	...
	judgeAgent, err := getAgent(ctx, judgeProv, judgeMod, cwd, "recac-arena-judge")
	if err != nil {
