...
// AgentFactory definition to match agentClientFactory signature
...
func NewArenaCmd(agentFactory AgentFactory) *cobra.Command {
	var (
		competitors string
		task        string
		file        string
...
Example:
  recac arena --competitors "openai:gpt-4,gemini:gemini-pro" --task "Explain quantum computing"`,
		RunE: func(cmd *cobra.Command, args []string) error {
			return runArena(cmd, agentFactory, competitors, task, file, judgeProv, judgeModel)
		},
	}
...
func runArena(cmd *cobra.Command, agentFactory AgentFactory, competitors, task, file, judgeProv, judgeModel string) error {
	...
	// Use a mutex to synchronize writes to stdout/stderr inside goroutines
	...
	for i, compStr := range cleanCompetitors {
		...
		go func(index int, cStr string) {
			...
			ag, err := agentFactory(ctx, provider, model, cwd, projName)
			if err != nil {
	...
	judgeAgent, err := agentFactory(ctx, judgeProv, judgeModel, cwd, "recac-arena-judge")
	if err != nil {
