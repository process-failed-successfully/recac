...
func generateHookScript(binPath, profile string) string {
	...
	script := []string{
		...
		"",
		"echo \"🔍 Running recac pre-commit checks...\"",
		"",
	}
	...
	for _, check := range checks {
		// Quote the binary path in the check command if it's using the variable
		quotedCheck := strings.Replace(check, "$RECAC_BIN", "\"$RECAC_BIN\"", 1)
		script = append(script, fmt.Sprintf("%s\nif [ $? -ne 0 ]; then\n  echo \"❌ Check failed: %s\"\n  exit 1\nfi", quotedCheck, strings.ReplaceAll(quotedCheck, "\"", "\\\"")))
	}

	script = append(script, "", "echo \"✅ All checks passed!\"", "exit 0")
