...
func saveInteractionJSONL(i Interaction, file string) {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error opening recording file: %v\n", err)
		return
	}
	defer f.Close()
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error marshaling interaction: %v\n", err)
		return
	}
	if _, err := f.Write(data); err != nil {
		fmt.Fprintf(os.Stderr, "Error writing to recording file: %v\n", err)
		return
	}
	f.WriteString("\n")
}
