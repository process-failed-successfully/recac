...
func runMockServer(cmd *cobra.Command, args []string) error {
	...
	// Create Handler
	handler := NewMockServerHandler(ctx, ag, contextContent, mockServerLatency)
...
func NewMockServerHandler(ctx context.Context, ag agent.Agent, contextContent string, latency time.Duration) http.HandlerFunc {
