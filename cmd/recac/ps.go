...
func newPsCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// --- Filter by Stale Time ---
			staleFilter, _ := cmd.Flags().GetString("stale")
			if staleFilter != "" {
				staleDuration, err := time.ParseDuration(staleFilter)
				if err != nil {
					return fmt.Errorf("invalid 'stale' value %q: must be a duration (e.g., '7d', '24h')", staleFilter)
				}
				staleTime := time.Now().Add(-staleDuration)

				var filteredSessions []unifiedSession
				for _, s := range allSessions {
					// A session is stale if it's not running, has an end time, and that end time is before the stale threshold.
					if s.Status != "running" && !s.EndTime.IsZero() && s.EndTime.Before(staleTime) {
						filteredSessions = append(filteredSessions, s)
					}
				}
				allSessions = filteredSessions
			...
			header := "NAME\tSTATUS\tLOCATION\tSTARTED\tDURATION\tLAST USED"
			if showCosts {
			...
			for _, s := range allSessions {
				...
				lastUsed := formatSince(s.EndTime)

				baseOutput := fmt.Sprintf("%s\t%s\t%s\t%s\t%s\t%s",
					s.Name, s.Status, s.Location, started, duration, lastUsed)
	...
	// Add flags to the new command instance
	cmd.Flags().BoolP("costs", "c", false, "Show token usage and cost information")
	cmd.Flags().String("sort", "time", "Sort sessions by 'cost', 'time', or 'name'")
	cmd.Flags().BoolP("errors", "e", false, "Show the first line of the error for failed sessions")
	cmd.Flags().Bool("remote", false, "Include remote Kubernetes pods in the list")
	cmd.Flags().String("status", "", "Filter sessions by status (e.g., 'running', 'completed', 'error')")
	cmd.Flags().String("since", "", "Filter sessions started after a specific duration (e.g., '1h', '30m') or timestamp ('2006-01-02')")
	cmd.Flags().Bool("show-diff", false, "Show git diff for the most recent or specified session")
	cmd.Flags().String("session", "", "Specify a session for --show-diff")
	cmd.Flags().String("stale", "", "Filter sessions that have been stopped for longer than a duration (e.g., '7d', '24h')")

	return cmd
}

// formatSince creates a human-readable "since" string from a time.
