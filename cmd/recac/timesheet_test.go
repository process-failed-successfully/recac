...
func TestGetGitCommits_AuthorWithSpecialChars(t *testing.T) {
	// 1. Setup Temp Git Repo
	tmpDir := t.TempDir()

	// Initialize git
	runGitHelper(t, tmpDir, "git", "init")
	runGitHelper(t, tmpDir, "git", "config", "user.email", "test@example.com")

	authorName := "test[bot]"
	runGitHelper(t, tmpDir, "git", "config", "user.name", authorName)
	...
	runGitHelper(t, tmpDir, "git", "add", "test.txt")
	runGitHelper(t, tmpDir, "git", "commit", "-m", "Initial commit")

	// 2. Call getGitCommits
	// We use "24h" which should cover the just created commit
	commits, err := getGitCommits(tmpDir, "24h", authorName)
	assert.NoError(t, err)

	// 3. Assert
	// Should find 1 commit. If regex bug exists, it will likely find 0 because "[bot]" won't match "test[bot]" literally
	assert.Len(t, commits, 1, "Should find 1 commit for author with special chars")
	if len(commits) > 0 {
		assert.Equal(t, authorName, commits[0].Author)
	}
}

func runGitHelper(t *testing.T, dir string, name string, args ...string) {
	t.Helper()
