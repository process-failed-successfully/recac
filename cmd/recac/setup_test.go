...
func TestSetupCmd_Bug_SubstringMatch(t *testing.T) {
	...
	// Create .env with a variable that ends with OPENAI_API_KEY
	os.WriteFile(".env", []byte("FAKE_OPENAI_API_KEY=fake\n"), 0600)
	defer os.Remove(".env")
	...
	assert.Contains(t, str, "FAKE_OPENAI_API_KEY=fake")

	// OPENAI_API_KEY should be added because it's a different key
