...
func TestSetupCmd_Bug_SubstringMatch(t *testing.T) {
	...
	// Create existing .env with a key that has the new key as a suffix
	// e.g., PREFIX_MY_KEY exists, and we want to add MY_KEY.
	os.WriteFile(".env", []byte("PREFIX_MY_KEY=foo\n"), 0600)
	defer os.Remove(".env")
	...
	// We want to force OPENAI_API_KEY to be added.
	// But let's say we have "NOT_OPENAI_API_KEY=foo" in .env
	os.WriteFile(".env", []byte("NOT_OPENAI_API_KEY=foo\n"), 0600)
	...
	content, _ := os.ReadFile(".env")
	str := string(content)
	assert.Contains(t, str, "NOT_OPENAI_API_KEY=foo")
	// This should be present, but due to the bug, it might be skipped
	// because "NOT_OPENAI_API_KEY=" contains "OPENAI_API_KEY="
	assert.Contains(t, str, "OPENAI_API_KEY=new-key")
}
