...
func TestSetupCmd_EnvPartialMatch(t *testing.T) {
	originalAskOne := askOneFunc
	defer func() { askOneFunc = originalAskOne }()

	// Create existing .env with a key that is a suffix of the key we want to add
	// e.g. MY_OPENAI_API_KEY exists. We want to add OPENAI_API_KEY.
	...
	viper.Reset()
	viper.SetConfigFile("test_config_partial.yaml")
	defer os.Remove("test_config_partial.yaml")
