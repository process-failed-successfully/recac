...
func TestSetupCmd_SubstringMatchRegression(t *testing.T) {
	...
	// Create existing .env with a key that is a SUPERSTRING of the new key
	// e.g. "MY_OPENAI_API_KEY" vs "OPENAI_API_KEY"
	os.WriteFile(".env", []byte("MY_OPENAI_API_KEY=some-other-key\n"), 0600)
	defer os.Remove(".env")
	...
	// We expect the new key to be appended because "MY_OPENAI_API_KEY" is NOT "OPENAI_API_KEY"
	assert.Contains(t, str, "OPENAI_API_KEY=new-key", "Should have appended OPENAI_API_KEY despite MY_OPENAI_API_KEY existing")
}
