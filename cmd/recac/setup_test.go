...
func TestSetupCmd_SubstringMatch_NonConflicting(t *testing.T) {
	originalAskOne := askOneFunc
	defer func() { askOneFunc = originalAskOne }()

	// Create existing .env with a key that contains the new key as substring
	os.WriteFile(".env", []byte("MY_API_KEY=foo\n"), 0600)
	defer os.Remove(".env")
	...
	content, _ := os.ReadFile(".env")
	str := string(content)
	assert.Contains(t, str, "MY_API_KEY=foo")
	assert.Contains(t, str, "OPENAI_API_KEY=new-key", "Should add OPENAI_API_KEY even if MY_API_KEY exists")
}
...
func TestSetupCmd_SubstringMatchRegression(t *testing.T) {
	...
	// Create existing .env with a key that contains the new key as substring
	os.WriteFile(".env", []byte("EXISTING_OPENAI_API_KEY=foo\n"), 0600)
	defer os.Remove(".env")
	...
	viper.Reset()
	viper.SetConfigFile("test_config_substring2.yaml")
	defer os.Remove("test_config_substring2.yaml")
	...
	content, _ := os.ReadFile(".env")
	str := string(content)
	assert.Contains(t, str, "EXISTING_OPENAI_API_KEY=foo")
	assert.Contains(t, str, "OPENAI_API_KEY=new-key", "Should add OPENAI_API_KEY even if EXISTING_OPENAI_API_KEY exists")
}
