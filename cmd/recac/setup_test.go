...
func TestSetupCmd_Bug_SubstringMatch(t *testing.T) {
	...
	// Create existing .env with a variable that ends with the key we are adding
	// e.g. "OTHER_OPENAI_API_KEY=foo"
	// If we add "OPENAI_API_KEY", it should NOT be skipped.
	os.WriteFile(".env", []byte("OTHER_OPENAI_API_KEY=foo\n"), 0600)
	defer os.Remove(".env")
	...
	// Expect OPENAI_API_KEY to be added because OTHER_OPENAI_API_KEY is not the same key
	assert.Contains(t, str, "OPENAI_API_KEY=new-key")
}
