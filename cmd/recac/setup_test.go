...
func TestSetupCmd_SubstringCollision(t *testing.T) {
	...
	// Create existing .env with a variable that has the key as a suffix
	// e.g. MY_OPENAI_API_KEY=foo
	...
	viper.Reset()
	viper.SetConfigFile("test_config_collision.yaml")
	defer os.Remove("test_config_collision.yaml")
	...
	content, _ := os.ReadFile(".env")
	str := string(content)
	// Expect OPENAI_API_KEY=new-key to be appended
	assert.Contains(t, str, "OPENAI_API_KEY=new-key")
}
