...
func TestSetupCmd_Bug_SubstringMatch(t *testing.T) {
	...
	// Create existing .env with a key that contains the new key as a substring (suffix)
	os.WriteFile(".env", []byte("EXPORT_OPENAI_API_KEY=old-key\n"), 0600)
	defer os.Remove(".env")
	...
	viper.Reset()
	viper.SetConfigFile("test_config_append_bug.yaml")
	defer os.Remove("test_config_append_bug.yaml")
	...
	content, _ := os.ReadFile(".env")
	str := string(content)
	assert.Contains(t, str, "EXPORT_OPENAI_API_KEY=old-key")
	assert.Contains(t, str, "OPENAI_API_KEY=new-key", "Should correctly append OPENAI_API_KEY even if EXPORT_OPENAI_API_KEY exists")
}
