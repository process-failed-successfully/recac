...
func NewCleanCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			fmt.Fprintln(cmd.OutOrStdout(), "Cleaning up temporary files...")

			tempFilesPath := "temp_files.txt"
			file, err := os.Open(tempFilesPath)
			if err != nil {
				if os.IsNotExist(err) {
					fmt.Fprintln(cmd.OutOrStdout(), "No temporary files to clean.")
					return
				}
				fmt.Fprintf(cmd.OutOrStdout(), "Error opening %s: %v\n", tempFilesPath, err)
				return
			}
			defer file.Close()
			...
			for _, f := range filesToRemove {
				...
				if err != nil {
					fmt.Fprintf(cmd.OutOrStdout(), "Error resolving path %s: %v\n", f, err)
					continue
				}
				err = os.Remove(absPath)
				if err != nil {
					if os.IsNotExist(err) {
						fmt.Fprintf(cmd.OutOrStdout(), "File %s already gone.\n", absPath)
					} else {
