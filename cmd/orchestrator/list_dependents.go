...
func listDependents(host, jobID string) {
	u, err := url.Parse(fmt.Sprintf("%s/jobs/%s/dependents", host, jobID))
	if err != nil {
	...
	if resp.StatusCode != http.StatusOK {
		...
		fmt.Fprintf(stdout, "Failed to fetch dependents: %s\n", strings.TrimSpace(string(body)))
		exitFunc(1)
		return
	}
	...
	if len(jobs) == 0 {
		fmt.Fprintf(stdout, "No dependents found for job %s.\n", jobID)
		return
	}
	...
	fmt.Fprintln(stdout, titleStyle.Render(fmt.Sprintf("Dependents of %s (%d)", jobID, len(jobs))))
	fmt.Fprintln(stdout, "")
