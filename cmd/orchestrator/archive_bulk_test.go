...
func TestArchiveBulkJobs(t *testing.T) {
	...
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.URL.Query().Get("tag") == "test-tag" || r.URL.Query().Get("match") == "test-match" || r.URL.Query().Get("status") == "Failed" || r.URL.Query().Get("older_than") == "24h" {
	...
	t.Run("Valid Older Than", func(t *testing.T) {
		buf.Reset()
		exitCode = 0
		outPath := filepath.Join(t.TempDir(), "test_older_than_archive.tar.gz")

		archiveBulkJobs(ts.URL, "", "", "", "", "24h", outPath)
