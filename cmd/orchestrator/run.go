...
var runCmd = &cobra.Command{
	...
	Long:  `Polls for work items and spawns agents according to configuration.`,
	Run: func(cmd *cobra.Command, args []string) {
		// Config
		config.Load(cfgFile)

		// Logger
		logger := telemetry.NewLogger(verbose, "orchestrator", false)
		telemetry.InitLogger(verbose, "orchestrator", false) // Ensure global logger is set
...
func init() {
	...
	runCmd.Flags().String("work-file", "work_items.json", "Work items file (for 'file' poller)")
	...
	viper.BindPFlag("orchestrator.work_file", runCmd.Flags().Lookup("work-file"))
