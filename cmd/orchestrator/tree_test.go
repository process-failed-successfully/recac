...
func TestPrintJobTree(t *testing.T) {
	tests := []struct {
		name       string
		jobID      string
		jobs       []orchestrator.JobInfo
		...
		{
			name:  "Linear Chain - Middle Job",
			jobID: "JOB-2",
			jobs: []orchestrator.JobInfo{
				{ID: "JOB-1", Status: "Completed", Summary: "Job One", WorkItem: orchestrator.WorkItem{DependsOn: []string{}}},
				{ID: "JOB-2", Status: "Active", Summary: "Job Two", WorkItem: orchestrator.WorkItem{DependsOn: []string{"JOB-1"}}},
				{ID: "JOB-3", Status: "Pending", Summary: "Job Three", WorkItem: orchestrator.WorkItem{DependsOn: []string{"JOB-2"}}},
				{ID: "JOB-4", Status: "Pending", Summary: "Job Four", WorkItem: orchestrator.WorkItem{DependsOn: []string{"JOB-3"}}},
				{ID: "JOB-99", Status: "Pending", Summary: "Unrelated", WorkItem: orchestrator.WorkItem{DependsOn: []string{}}},
			},
			expected: []string{
				"Job Dependency Tree for JOB-2 (4 Jobs)",
				"JOB-1",
				"JOB-2",
				"JOB-3",
				"JOB-4",
		...
		{
			name:  "Complex Tree - Target Branch",
			jobID: "JOB-3",
			jobs: []orchestrator.JobInfo{
				{ID: "JOB-1", Status: "Completed", Summary: "Job One"},
				{ID: "JOB-2", Status: "Completed", Summary: "Job Two", WorkItem: orchestrator.WorkItem{DependsOn: []string{"JOB-1"}}},
				{ID: "JOB-3", Status: "Failed", Summary: "Job Three", WorkItem: orchestrator.WorkItem{DependsOn: []string{"JOB-1"}}},
				{ID: "JOB-4", Status: "Pending", Summary: "Job Four", WorkItem: orchestrator.WorkItem{DependsOn: []string{"JOB-2", "JOB-3"}}},
				{ID: "JOB-5", Status: "Pending", Summary: "Job Five", WorkItem: orchestrator.WorkItem{DependsOn: []string{"JOB-2"}}}, // Not in JOB-3's path
			},
			expected: []string{
				"Job Dependency Tree for JOB-3 (3 Jobs)",
				"JOB-1",
				"JOB-3",
				"JOB-4",
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Execute
			printJobTree(server.URL, tt.jobID)
...
func TestTreeJobCommandFlag(t *testing.T) {
	mux := http.NewServeMux()
	mux.HandleFunc("/jobs", func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		json.NewEncoder(w).Encode([]orchestrator.JobInfo{
	...
	viper.Set("orchestrator.tree_job", "JOB-1")
	viper.Set("orchestrator.host", server.URL)
	...
	assert.Contains(t, out.String(), "Job Dependency Tree for JOB-1")
	assert.Contains(t, out.String(), "JOB-1")
}
