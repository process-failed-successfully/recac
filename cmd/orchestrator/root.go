...
var (
	...
	rootCmd = &cobra.Command{
		...
		Long:  `Polls Jira/GitHub/Files and spawns RECAC Agents.`,
		Run:   runOrchestrator,
	}
)
...
func init() {
	...
	f := rootCmd.Flags()
	f.String("mode", "local", "Orchestrator mode: 'local' (Docker) or 'k8s' (Kubernetes Job)")
	f.String("jira-label", "recac-agent", "Jira label to poll for")
	f.String("image", "ghcr.io/process-failed-successfully/recac-agent:latest", "Agent image to spawn")
	f.String("namespace", "default", "Kubernetes namespace (for k8s mode)")
	f.Duration("interval", 1*time.Minute, "Polling interval")
	f.String("agent-provider", "openrouter", "Provider for spawned agents")
	f.String("agent-model", "meta-llama/llama-3.3-70b-instruct:free", "Model for spawned agents")
	f.String("image-pull-policy", "Always", "Image pull policy for agents (Always, IfNotPresent, Never)")

	f.String("jira-query", "", "Custom JQL query (overrides label)")
	f.String("poller", "jira", "Poller type: 'jira', 'github', 'file', or 'file-dir'")
	f.String("work-file", "work_items.json", "Work items file (for 'file' poller)")
	f.String("watch-dir", "", "Directory to watch for work item files (for 'file-dir' poller)")

	f.String("github-token", "", "GitHub API Token (for 'github' poller)")
	f.String("github-owner", "", "GitHub Repository Owner (for 'github' poller)")
	f.String("github-repo", "", "GitHub Repository Name (for 'github' poller)")
	f.String("github-label", "", "GitHub Label to poll for (defaults to jira-label if not set)")

	viper.BindPFlag("verbose", rootCmd.PersistentFlags().Lookup("verbose"))

	viper.BindPFlag("orchestrator.jira_query", f.Lookup("jira-query"))
	viper.BindPFlag("orchestrator.poller", f.Lookup("poller"))
	viper.BindPFlag("orchestrator.work_file", f.Lookup("work-file"))
	viper.BindPFlag("orchestrator.watch_dir", f.Lookup("watch-dir"))

	viper.BindPFlag("orchestrator.github_token", f.Lookup("github-token"))
	viper.BindPFlag("orchestrator.github_owner", f.Lookup("github-owner"))
	viper.BindPFlag("orchestrator.github_repo", f.Lookup("github-repo"))
	viper.BindPFlag("orchestrator.github_label", f.Lookup("github-label"))

	viper.BindPFlag("orchestrator.mode", f.Lookup("mode"))
	viper.BindPFlag("orchestrator.jira_label", f.Lookup("jira-label"))
	viper.BindPFlag("orchestrator.image", f.Lookup("image"))
	viper.BindPFlag("orchestrator.namespace", f.Lookup("namespace"))
	viper.BindPFlag("orchestrator.interval", f.Lookup("interval"))
	viper.BindPFlag("orchestrator.agent_provider", f.Lookup("agent-provider"))
	viper.BindPFlag("orchestrator.agent_model", f.Lookup("agent-model"))
	viper.BindPFlag("orchestrator.image_pull_policy", f.Lookup("image-pull-policy"))
