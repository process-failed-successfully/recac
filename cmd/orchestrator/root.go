...
var rootCmd = &cobra.Command{
	Use:   "orchestrator",
	Short: "RECAC Orchestrator for managing autonomous coding agents",
	Run:   runOrchestrator,
}
...
func init() {
	...
	pflags.StringVar(&cfgFile, "config", "", "config file (default is $HOME/.recac.yaml)")
	pflags.BoolP("verbose", "v", false, "Enable verbose/debug logging")

	// Orchestrator Flags
	pflags.String("mode", "local", "Orchestrator mode: 'local' (Docker) or 'k8s' (Kubernetes Job)")
	pflags.String("jira-label", "recac-agent", "Jira label to poll for")
	pflags.String("image", "ghcr.io/process-failed-successfully/recac-agent:latest", "Agent image to spawn")
	pflags.String("namespace", "default", "Kubernetes namespace (for k8s mode)")
	pflags.Duration("interval", 1*time.Minute, "Polling interval")
	pflags.String("agent-provider", "openrouter", "Provider for spawned agents")
	pflags.String("agent-model", "meta-llama/llama-3.3-70b-instruct:free", "Model for spawned agents")
	pflags.String("image-pull-policy", "Always", "Image pull policy for agents (Always, IfNotPresent, Never)")

	pflags.String("jira-query", "", "Custom JQL query (overrides label)")
	pflags.String("poller", "jira", "Poller type: 'jira', 'github', 'file', or 'file-dir'")
	pflags.String("work-file", "work_items.json", "Work items file (for 'file' poller)")
	pflags.String("watch-dir", "", "Directory to watch for work item files (for 'file-dir' poller)")

	pflags.String("github-token", "", "GitHub API Token (for 'github' poller)")
	pflags.String("github-owner", "", "GitHub Repository Owner (for 'github' poller)")
	pflags.String("github-repo", "", "GitHub Repository Name (for 'github' poller)")
	pflags.String("github-label", "", "GitHub Label to poll for (defaults to jira-label if not set)")
...
func bindFlags() {
	// We bind flags in init() so they are ready when Run is called.
	// Note: Viper binds to the *flag set*, not the command.
	// Since we use PersistentFlags, they are available on rootCmd and children.

	// Helper to bind a flag to a viper key
	bind := func(key string, flag string) {
		viper.BindPFlag(key, rootCmd.PersistentFlags().Lookup(flag))
	}

	bind("verbose", "verbose")
	bind("orchestrator.jira_query", "jira-query")
	bind("orchestrator.poller", "poller")
	bind("orchestrator.work_file", "work-file")
	bind("orchestrator.watch_dir", "watch-dir")

	bind("orchestrator.github_token", "github-token")
	bind("orchestrator.github_owner", "github-owner")
	bind("orchestrator.github_repo", "github-repo")
	bind("orchestrator.github_label", "github-label")

	bind("orchestrator.mode", "mode")
	bind("orchestrator.jira_label", "jira-label")
	bind("orchestrator.image", "image")
	bind("orchestrator.namespace", "namespace")
	bind("orchestrator.interval", "interval")
	bind("orchestrator.agent_provider", "agent-provider")
	bind("orchestrator.agent_model", "agent-model")
	bind("orchestrator.image_pull_policy", "image-pull-policy")

	// Env vars are bound globally via Viper
