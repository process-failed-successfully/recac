...
*   **Local Execution**: When `recac` runs locally and targets a remote cluster, we cannot "bind mount" the local workspace easily.
    *   *Solution A (MVP)*: Assume `recac` runs **inside** the cluster (e.g., as a pod itself) and shares a PersistentVolumeClaim (PVC) with the agent Job.
    *   *Solution B (Remote)*: Use `kubectl cp` (via client-go `SPDY` executor) to sync files up/down. This is slow for large repos.
    *   *Solution C (Git)*: The agent Job clones the repo from the remote origin. `recac` manages the agent via Git ops (pushing changes).
